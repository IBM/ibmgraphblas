...
// column index)and column indices plus one (in algo_util_read_matrix.c).
...
int main (int argc, char **argv)
{
    ...
    // print input
    sr_dist_print_matrix (A, "A(input)");

    // initialize Dist Semiring
    OK (Dist_init());

    // compute SSSP
    GrB_Index source = 0;
    bool no_neg_cycle;
    OK (algo_sssp_dist (A, source, &p_d, &no_neg_cycle));

    // print results
    sr_dist_print_vector (p_d, "d(result)");
